defmodule Etso.Adapter.Behaviour.Schema do
  ...
  @impl Ecto.Adapter.Schema
  def insert_all(%Meta{} = meta, %{schema: schema}, _, entries, _, _, _, _) do
  ...
  @impl Ecto.Adapter.Schema
  def insert(%Meta{} = meta, %{schema: schema}, fields, _, _, _) do
    {:ok, ets_table} = Etso.Registry.ensure(meta, schema)
    ets_field_names = TableStructure.field_names(schema)
  ...
  @impl Ecto.Adapter.Schema
  def update(%Meta{} = meta, %{schema: schema}, fields, filters, [], _) do
    {:ok, ets_table} = Etso.Registry.ensure(meta, schema)
  ...
  @impl Ecto.Adapter.Schema
  def delete(%Meta{} = meta, %{schema: schema}, filters, _) do
    {:ok, ets_table} = Etso.Registry.ensure(meta, schema)
    [key_name] = schema.__schema__(:primary_key)
    [{^key_name, key}] = filters
